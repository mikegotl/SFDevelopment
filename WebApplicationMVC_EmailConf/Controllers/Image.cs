...
namespace WebApplicationMVC_EmailConf.Controllers
{
    public class ImageUpload
    {
        ...
        private readonly string UploadPath = "~/Content/Images/members/profilepics";

        public ImageResult RenameUploadFile(HttpPostedFileBase file, Int32 counter = 0)
        {
            var fileName = Path.GetFileName(file.FileName);

            string prepend = MyHelpers.LoggedInMember.MemberID.ToString() + "_image_";
            string finalFileName = prepend + ((counter).ToString()) + "_" + fileName;
            string path = Path.Combine(HttpContext.Current.Request.MapPath(UploadPath) + "/" + finalFileName);
